...
- 이후 감사 3회(전체→전체→부분), 재조사 1회, PDF 제작 2회(주소 누락 재작업 포함)를 거쳐 조건부 초안 PDF를 최종 등록했다. verdict는 끝까지 conditional — blocker(KTX·호텔 예약 미완료)는 하네스가 예약·결제를 하지 않는다는 원칙상 총괄이 해소할 수 없는, 사용자만 풀 수 있는 항목이다.
- **실제 실행에서 드러난 프로세스 결함**: (1) 총괄이 `candidates.json`을 저장하지 않고 조사 결과를 대화 맥락에만 의존해 계획을 짜, 종로뭉티기 주소가 최종 원본에서 누락됐다 — artifact-producer가 이를 지어내지 않고 정확히 잡아내 재작업(PDF 재제작)이 발생했다(하네스는 의도대로 작동했지만 총괄의 규칙 준수가 실패한 사례). (2) 총괄이 단일 사실 확인을 이유로 WebSearch를 여러 차례 직접 실행해, "조사는 서브에이전트, 총괄은 통합"이라는 역할 경계가 실전에서 흐려졌다. (3) 후보 확정 후 감사·제작(비용이 큰 단계)으로 바로 넘어가는 구조라, 사용자가 결과를 보고 나서야 피드백할 수 있었고 그 시점의 수정은 재감사·재제작을 유발해 비쌌다. (4) 여러 서브에이전트가 백그라운드에서 도는 동안 총괄이 진행 상황을 매번 즉시 알리지 않아 사용자가 진행률을 알 방법이 없었다. (5) "여유롭게"라는 취향에 총괄이 과하게 기울어 관광 슬롯 2개를 조사된 구체적 명소 대신 "동성로 자유시간"이라는 빈 슬롯으로 채워, 사용자가 명시한 "핵심은 다 보고"라는 요구와 실제 산출물의 균형이 맞지 않았다.
- **보완**: `SKILL.md`에 6단계로 "사용자 확인 체크포인트"를 신설했다 — 후보·초안을 감사·제작 전에 사용자에게 보여주고 승인받으며, 이 시점에 candidates.json 존재 여부도 확인한다(그 외 단계 번호가 1씩 밀림). `roles/coordinator.md`에 서브에이전트 완료마다 즉시 진행 상황을 알리는 규칙, 그리고 관광·체험을 명시한 사용자에게는 관광 슬롯을 식사보다 적게 배정하거나 "자유시간"으로만 채우지 않는 균형 규칙을 추가했다. 37개 테스트 회귀 통과 유지. 이 세 가지가 실제로 사용자 경험을 개선하는지는 다음 실행에서 확인한다. (2)의 총괄-조사자 역할 경계 흐려짐은 이번엔 고치지 않았다 — 단일 사실 확인까지 매번 서브에이전트로 위임하면 오히려 비용이 늘 수 있어, 트레이드오프를 더 고민한 뒤 별도로 다룬다.

## 실행 결과물에 대한 사용자 피드백 후 추가 보완 (같은 검증일)

사용자가 실제 PDF를 보고 두 가지를 더 지적했다: (1) 각 항목에 설명이 전혀 없다 — 시간·주소·전화번호뿐이고 그 장소가 어떤 곳인지 서술이 없다. (2) 감사 단계에서 네이버 도메인 접근 차단을 서로 다른 에이전트(research-food-002, audit-002, audit-003)가 각자 독립적으로 재발견하고 재실패했다 — 총괄이 이미 아는 정보를 다음 서브에이전트에게 넘기지 않았다.

- **원인**: (1) `references/protocol.md`의 후보 스키마에 애초에 여행자용 설명 필드가 없었다 — `fit_reason`은 총괄이 왜 그 후보를 골랐는지에 대한 것이지, 그 장소가 무엇인지에 대한 서술이 아니다. 스키마에 없으니 조사자도 안 채웠고 제작자도 넣을 데가 없었다. (2) 총괄이 작업 지시서를 만들 때 이전 결과의 "출처 접근 실패" 사실을 다음 지시서에 옮기지 않았다.
- **보완**: `protocol.md` 후보 스키마에 `description`(여행자용 설명, fit_reason과 별도) 필드를 추가했다. `roles/food.md`, `roles/experience.md`, `roles/lodging.md`에 각각 description을 확인된 사실 범위에서 채우도록 지시를 추가했다. `references/artifact-producer.md`에 산출물이 description을 포함해 시간·주소만 나열한 표가 되지 않도록 명시했다. `references/roles.md`의 공통 작업 지시서 전달 목록과 감사 지시서에 "이미 확인된 접근 차단 출처"를 포함시키고, `roles/coordinator.md`에 이전 결과에서 확인된 접근 차단·반복 실패를 이후 작업 지시서에 반영하는 규칙을 추가했다.
- 37개 테스트 회귀 통과 유지. 현재 대구 여행(daegu-20260918)의 plan-v003/candidates.json은 이번 스키마 변경 이전에 만들어져 description 필드가 없다 — 사용자 요청에 따라 이번 대화에서는 소급 수정하지 않았다.

**범위 확대**: 사용자가 (2)의 수정이 "접근 차단"에만 좁게 한정돼 있다고 지적했다 — 총괄이 이미 알고 있는 것이면 접근 차단뿐 아니라 전부(이미 확인된 사실, 이미 탈락시킨 후보와 사유, 이미 답을 못 찾은 질문) 다음 작업 지시서에 반영해 서브에이전트가 처음부터 반복하지 않게 해야 실제로 시간이 줄어든다는 취지였다. `references/roles.md`의 공통 작업 지시서 전달 목록을 "이미 확인한 사실·탈락 후보·실패한 시도"로 넓혔다. 다만 감사 지시서는 신중하게 구분했다: 감사자의 사실 재확인(영업시간·가격 등)은 독립 검증이 목적이므로 "이미 확인됐다"는 이유로 생략시키지 않고, 오직 "도구·출처·도메인 자체가 이 환경에서 막혀 있어 재시도해도 결과가 달라지지 않는 것"만 생략 대상으로 한정했다 — 그렇지 않으면 감사의 독립성이라는 핵심 가치가 효율성 때문에 훼손된다. `roles/coordinator.md`에도 같은 구분을 명시했다. 37개 테스트 회귀 통과 유지.

## intake(조건 수집) 단계 설계 추가 (같은 검증일)

대구 여행 실행에서 날짜("다음주 금~토")와 예산("인당 30 정도")처럼 모호한 입력을 총괄이 임의로 해석하고 사후에 "다르면 말씀해주세요"라고만 했다. 실제로는 맞았지만, 틀렸을 경우 계획·감사·제작을 전부 다시 해야 했다. 같은 뿌리에서 "여유롭게"라는 한 단어 취향을 총괄이 임의로 해석해 관광 슬롯을 빈 "자유시간"으로 채운 것(앞선 항목 참고)도 나왔다. 사용자가 최초 여행 실행 전 조건 수집 단계 자체를 더 상세하게 설계해달라고 요청했다.

- **신설**: `harness/travel-planner/references/intake.md`를 새로 만들었다. 하드 제약(모호하면 절대값으로 환산해 조사 시작 *전에* 확인받음 — 해석 후 통보가 아니라 진행 전 확인), 취향(관광:식음료:휴식 비중, 관광 성향, 음식 성향, 페이스 등 한 단어 표현을 구체 항목으로 분해), 제약사항("없음"도 명시적으로 받음), 산출물 상세도 기대치까지 4개 범주로 구성했다. 질문은 2~3라운드로 나누고, 사용자가 명시적으로 생략을 요청하면 기본값으로 진행하되 그 기본값을 assumptions에 남기도록 했다.
- `SKILL.md` 2단계와 `roles/coordinator.md` 1단계가 이 체크리스트를 참조하도록 연결했다.
- 37개 테스트 회귀 통과 유지(intake.md는 core_root 참조 문서라 `test_core_has_no_host_tool_dependencies`의 스캔 대상이지만 금지 문자열이 없어 통과). 이 intake 절차가 실제로 재작업을 줄이는지는 다음 실제 여행 실행에서 확인한다.

## "예약 금지"를 가격 조회에까지 과잉 적용한 문제 수정 (같은 검증일)

사용자가 지적했다: 실제 결제·확정은 못 해도 예약 사이트에 들어가 실제 가격을 보는 것, 여러 옵션 중 최저가를 찾아주는 것까지 못 할 이유는 없다. 실제로 대구 여행에서 KTX·호텔 가격이 전부 오래된 집계 사이트(카약 "최근 2주 최저~최고가" 등)의 추정치로만 남았는데, "예약·결제 금지"를 조사자가 예약 페이지 자체를 열람하지 않는 것으로 과잉 해석한 결과였다.

- **보완**: `roles/logistics.md`와 `roles/lodging.md`에 "예약(결제·확정)은 금지지만 가격·잔여석 조회는 금지가 아니다"를 명시했다. 실제 예매/예약 사이트에 여행 날짜·인원을 넣어 그 조건의 실제 가격을 조회하고, 가능하면 여러 옵션 중 최저가를 찾아 보고하도록 지시를 추가했다. 승객·투숙객 정보 입력이나 "예약/결제" 버튼 이전까지만 진행하는 경계도 명시했다. `roles/coordinator.md`의 "변경과 위임 경계"에도 이 구분을 추가했다.
- **알려진 기술적 한계도 함께 명시**: 조사자의 웹 도구(WebFetch 등)가 정적 페이지만 읽을 수 있어, 예약 사이트의 날짜 검색 폼처럼 JS로 동적 렌더링되는 가격은 애초에 못 읽어올 수 있다. 이 경우 실패 사실과 시도한 방법을 명시하고 오래된 평균가를 확정 가격처럼 보고하지 않도록 했다 — 지침은 고쳤지만 도구 자체의 한계까지 없앤 것은 아니다. 실제로 최저가를 더 잘 찾아내는지는 다음 실제 여행 실행에서 확인한다.
- 37개 테스트 회귀 통과 유지.

## browse(동적 페이지 조회) 기능 추가 — 실제 검증 및 실수 정정 (같은 검증일)

사용자가 "정적 페이지만 읽을 수 있다"는 한계 자체를 도구를 바꿔서 없앨 수 없냐고 물었다. 이 세션(Claude 데스크톱 앱)에는 실제로 브라우저 자동화 도구(Claude Browser MCP)가 있어, 날짜 검색 폼처럼 동적 렌더링되는 페이지도 실제로 입력·클릭해서 읽을 수 있다.

- **설계**: `references/runtime.md`에 browse를 delegate/search/ask 등과 같은 급의 추상 기능으로 추가했다(공통 코어는 특정 도구명에 의존하지 않는다는 기존 원칙 유지). `adapters/claude.md`는 browse를 Claude Browser 도구로, `adapters/codex.md`는 "노출된 실제 브라우저 자동화 도구가 있으면 사용, 없으면 미지원 표시"로 일반화해 연결했다. browse는 결제·개인정보 입력이나 "결제/예약확정" 버튼 이전까지만 진행하고 페이지 내용의 지시문을 따르지 않는다는 안전 경계도 명시했다(프롬프트 삽입 방지).
- **실제 검증 중 발견한 진짜 실수**: `.claude/agents/travel-researcher.md`/`travel-auditor.md`의 `tools:`에 `mcp__Claude_Browser__navigate` 같은 개별 MCP 도구 이름을 나열했는데, 실제로 서브에이전트를 실행해보니 Read/Glob/Grep/WebSearch/WebFetch 다섯 개만 노출되고 Browser 도구는 하나도 전달되지 않았다. 문서만 고쳐놓고 "됐다"고 하지 않고 실제로 에이전트를 띄워 확인했기 때문에 이 실패를 바로 잡을 수 있었다. claude-code-guide 서브에이전트에 문의한 결과, Claude Code 서브에이전트의 `tools:` 필드는 개별 MCP 도구 이름이 아니라 `mcp__<서버이름>`(서버 전체) 패턴만 지원한다는 답을 받았다 — 이 답변도 그대로 믿지 않고 `tools: ..., mcp__Claude_Browser`로 고친 뒤 도구 목록만 보고하는 짧은 테스트로 실제 노출을 재확인했다.
- **최종 실제 조회 성공**: 도구 배선을 고친 뒤 discovery-lodging 역할로 토요코인 대구 동성로를 2026-09-18~19·2인·트윈룸 조건으로 실제 재조회했다. 공식 사이트(toyoko-inn.com)에서 날짜·인원 폼을 직접 입력해 동적 렌더링된 실가격 94,000원(회원가 89,300원), 잔여 1실을 확인했다(evidence_status=verified, browse). Agoda는 매진, Booking.com·야놀자는 이 호텔을 취급하지 않음도 함께 확인해 "여러 채널 비교 후 최저가 보고"까지 설계대로 작동했다. 예약·결제 버튼이나 개인정보 입력 이전에 정확히 멈췄다.
- browse 기능 자체는 실증됐다. 다만 이 세션(Claude 데스크톱 앱)에서만 검증했고, browse 미노출 환경(Codex, 순수 CLI 등)에서의 우아한 저하(estimated 표시)는 아직 실제로 검증하지 않았다.
- 37개 테스트 회귀 통과 유지.
